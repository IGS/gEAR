...
# For a given BED6 file of genes, convert to BED12 format by adding exons
# from a BED6 exon.bed file as a list of blockSizes and blockStarts
...
# Second file: gene_bed
{
    ...
    if (exonCount[g_name] > 0) {
        ...
        starts = ""
        for (i = 1; i <= count; i++) {
            size = exonEnd[g_name,i] - exonStart[g_name,i]
            start = exonStart[g_name,i] - chromStart
            sizes = sizes ((i==1) ? "" : ",") size
            starts = starts ((i==1) ? "" : ",") start
        }
        blockCount = count
        print chrom, chromStart, chromEnd, g_name, g_score, g_strand, chromStart, chromEnd, "0,0,0", blockCount, sizes, starts
    } else {
