...
| `check_dataset_processing_status.cgi` | not called by `upload_dataset.js` | Older CGI version of the status endpoint. Reads `status.json` and runs the legacy `process_id` check. | `session_id`, `share_uid` | none |
