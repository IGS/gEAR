...
"""
uploader_consumer.py - RabbitMQ messaging consumer
"""
...
def _on_request(channel, method_frame, properties, body):
    ...
    share_id = deserialized_body["share_id"]
    session_id = deserialized_body["session_id"]

    with open(stream, "a") as fh:
        print("{} - [x] - Received request for session_id {} and share_id {}".format(pid, session_id, share_id), flush=True, file=fh)
        import pika
        ...
        try:
            ...
            print("{} - [x] - Publishing response for session_id {} and share_id {}".format(pid, session_id, share_id), flush=True, file=fh)
...
def main():
    ...
    host = this.servercfg['uploader_service']['queue_host']
