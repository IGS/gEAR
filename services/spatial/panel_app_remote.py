...
# Pathlib does not work well with non-local filestores, so use os.path.join for safety
url_root = "https://devel.umgear.org"
spatial_path = os.path.join(url_root, 'datasets/spatial')
...
def check_url(url):
    try:
        ...
        raise ValueError(f"Invalid or inaccessible URL: {zarr_uri}")
...
class SpatialPanel(pn.viewable.Viewer):
    ...
    def prep_sdata(self):
        zarr_uri = os.path.join(spatial_path, f"{self.dataset_id}.zarr")

        check_url(zarr_uri)

        sdata = sd.read_zarr(zarr_uri, selection=["images", "tables"])
