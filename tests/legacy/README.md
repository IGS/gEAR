...
These suites predate the current site and are **no longer maintained or run in CI**. They are kept
for reference; parts may be useful when writing new UI tests. The maintained, CI-run tests are the
server-side pytest suite in [`tests/python`](../python) (see the
[testing guide](../../docs/developer/testing.md)).

| Suite | Files | Status |
| --- | --- | --- |
| Mocha + Playwright | `test/*.test.js`, `test/helpers.js`, `package.json` | Runs against `https://devel.umgear.org` (or `LOCAL=true` for `http://localhost:8080`). The `mocha_tests.yml` workflow is manual-only (`workflow_dispatch`). |
| SeleniumBase / pytest | `test_*.py` | Written for the v1 UI (2022); runs against a live site. |
| Legacy runner | `run_tests`, `test_commands.json`, `accounts__*.py`, `datasets__*.py`, `gene_cart_manager__*.py`, `index__*.py` | Raw Selenium scripts (2021-22) for the v1 UI. |

Fixture files these suites used remain in [`tests/data`](../data).

## UI test suites

There are two UI test suites in this directory:

- **Mocha + Playwright** (JavaScript, `tests/legacy/test/*.test.js`) - described below. Most API calls are mocked.
- **pytest + SeleniumBase** (Python, `tests/legacy/test_*.py`) - runs against a live site. See the [testing guide](../../docs/developer/testing.md) for details.
...
Ensure npm is installed, and make sure you are in the `<gear_root>/tests/legacy` directory. Dependencies are already declared in `tests/legacy/package.json` (`mocha`, `@playwright/test`), so just run:
...
`npm test` runs `mocha`, which uses the Mocha default spec `./test/*.{js,cjs,mjs}` (non-recursive, so `tests/legacy/test/e2e/` is not included). Assertions use `expect` from `playwright/test`; Chai is not used. Shared helpers and API mocks are in `tests/legacy/test/helpers.js`.
