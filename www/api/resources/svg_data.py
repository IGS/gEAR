    ...
    def get(self, dataset_id):
        ...
        scores = {
          "dataset": {
            ...
            "min": round(float(adata.X.min())),
            "max": round(float(adata.X.max()))
          },
          "gene": {
            "gene": gene_symbol,
            "min": round(float(selected.X.min())),
            "max": round(float(selected.X.max()))
          },
          "tissue": dict()
        }
        ...
        for tissue in tissues:
          ...
          scores['tissue'][tissue] = {
            "min": round(float(tissue_adata.X.min())),
            "max": round(float(tissue_adata.X.max()))
          }
