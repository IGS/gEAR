...
class DatasetCollectionPanel {
    constructor({datasets, layout_id, layout_label} = {}) {
        ...
        // Add a listener to handle resizing plots when window is resized.
        var dsp = this;
        // $(window).resize(dsp, dsp.redraw_plotly_plots);
        /*
        // SAdkins - 2020/8/14 - Commenting out since this seems to not work.
        // The site in general is not very responsive so removing this isn't the worst right now
        $(window).on('resize', function() {
            dsp.redraw_plotly_plots(dsp);
        });
        */
    }
            ...
            success: function(data, textStatus, jqXHR) {
                $.each( data['datasets'], function(i, ds) {
                    ...
                    if (dataset.load_status == 'completed') {
                        ...
                        // Insert line-breaks into dataset descriptions
                        if (dataset.ldesc) {
